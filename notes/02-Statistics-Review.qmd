...
which is a necessary condition for ergodicity. For (weakly) stationary time series the 
...
\gamma(1) & = Cov(X_1, X_2) = Cov(X_2, X_3) = \ldots = Cov(X_{t}, X_{t + 1}) = Cov(X_{T - 1}, X_{T1}) \\
...
$$
\hat{\mu} = \frac{1}{T}\sum_{t = 1}^{T} =: \bar{x}
$$
...
And the covariances with the _empirical_ covariances:
...
dt_sim <- tibble(
  ...
  mutate(
    x_below_avg = x < mean(x),
    y_below_avg = y < mean(y),
    xy_gr = x_below_avg == y_below_avg
  )
mean(dt_sim$x)
mean(dt_sim$y)
```
...
dt_sim %>%
  ggplot(aes(x = x, y = y)) +
  geom_point(aes(color = x_below_avg, shape=y_below_avg)) + 
  geom_vline(xintercept=mean(dt_sim$x), lty=2, alpha = 0.5) + 
  geom_hline(yintercept=mean(dt_sim$y), lty=2, alpha = 0.5)
```

```{r}
cov(dt_sim$x, dt_sim$y)
cor(dt_sim$x, dt_sim$y)
```
...
3. Construct a two new variables that contain the first and the second lags of `Close`. Hint: Use the `mutate` and `lag` functions.
...
6. Use the `acf` function to plot the autocorrelation function of the `Close` time series.
