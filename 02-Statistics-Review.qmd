...
:::{#exm-sampling-x}
## Samling from the distribution of $X$
...
The variance of a random variable measures how different are (XXX) the possible values that can occur. It is the expected value of the squared diviations from the expected value.
...
Compute the conditional distribution of $Y$, given $X = 0$.
...
What you should see in @tbl-xy-distr-ind is that the conditional distributions of $Y$ are the same for every possible value of $X$. This is of course a consequence of the way we constructed this joint distribution in the first place: namely, we assumed that $X$ and $Y$ are independent.
...
#| tbl-cap: "Conditional distributions of"
...
::: {#exr-conditional-y}
What are the conditional distributions of $X$ given $Y$?
:::
...
:::{#exr-cond-exp-ind}
Calculate the expected value of $Y$ given $X$ for every possible value of $X$ in the case joint distribution under independence (XXX).
:::
...
:::{#exr-covariance}
## Covariance
Compute the covariance of $X$ and $Y$ under the joint distributions given in @tbl-distr-indep and @tbl-distr-dep.
