...
$$
y_{t} = \phi_0 + \phi_1 x_{t - 1} + e_t, e_t \sim WN(\sigma^2)
$$
...
$$
y_t = \phi_0 + e_t + \theta_1 e_{1 - 1}
$$

The conditional (given the history up to and includnig $t$) mean of $y_{t + h}$ is:

$$
E_ty_{t + h} = \phi_0 + E_t(e_{t + h}) + \theta_1 E_t (e_{t + h - 1})
$$
...
$$
\hat{y}_{0}(1) = \phi_0 + \phi_1 x_0 + \theta_1 e_0\\
$$
...
$$
\phi_0 y_t = 0.1 \phi_0 y_{t−1} + e_t + 0.2 e_{t−1}
$$

Compute the one-step ahead forecast of the model given the values

| t   | y   | $x _t = \phi_0 y_{t}$ | $\hat{x}_{t - 1}(1)$                            | $\hat{e}_{t}$            |
|-----|-----|-----------------------|-------------------------------------------------|--------------------------|
| -1  |     |                       |                                                 |                          |
| 0   | -11 | NA                    |                                                 |                          |
| 1   | 2   | 13                    |                                                 | $0$                      |
...
$$
\phi_0 y_t = 0.1 \phi_0 y_{t−1} + 0.3 \phi_0 y_{t - 2} + e_t + 0.2 e_{t−1}
$$

Again, set $x_t = \phi_0 y_{t}$ to get a regular ARMA(2, 1) model.
...
$$
\phi_0 y_t = 0.1 \phi_0 y_{t−1} + e_t + 0.2 e_{t−1} + 0.3 e_{t -2}
$$

Substituting $\phi_0 y_{t} = x_t$:
