...
A *time series* is a sequence of values ordered by time. In this course,
we will focus on series that take values in $\mathbb{R}$. Let
...
Internally, dates are stored as the number of days since a reference
epoch (origin). The default reference in R is the first of January 1970.
...
classes. You can find out more about the package
...
available in the internal market in
...
An advantage of the `tsibble` objects is that you can use familiar verbs
for example
...
1.  Create a `tsibble` object to hold the time series. Hint: use the
    `as_tsibble` and `yearquarter` functions from the `tsibble` package.
